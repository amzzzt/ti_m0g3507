/**
 * ball_twopoint.c — 双点序列: +118(停1s) → -113(停1s)
 */
...
enum { S_TO_P118, S_HOLD1, S_TO_N113, S_HOLD2, S_DONE };

void ball_twopoint_init(void)
{
    lt = tick_get();
    ...
    ball_control_set_target(&b, 118.0f);
    st = S_TO_P118;
}

void ball_twopoint_update(void)
{
    ...
    case S_TO_P118:
        if (ball_control_is_ok(&b)) {
            float ae = (b.dx_f > 118.0f) ? (b.dx_f - 118.0f) : (118.0f - b.dx_f);
    ...
    case S_TO_N113:
        if (ball_control_is_ok(&b)) {
            float ae = (b.dx_f > -113.0f) ? (b.dx_f - (-113.0f)) : ((-113.0f) - b.dx_f);
            float av = (b.vx > 0 ? b.vx : -b.vx);
            if (ae < 15.0f && av < 2.0f) {
                settle++;
                if (settle >= 50) {
